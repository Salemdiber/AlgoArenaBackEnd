...
export class ChallengeService {
  ...
  private async validateDuplicateChallenge(
    ...
    const exactTitleMatch = await this.challengeModel
      .findOne({ title })
      .lean()
      .exec();
    if (
      exactTitleMatch &&
      String((exactTitleMatch as any)._id) !== String(ignoreId || '')
    ) {
      throw new ConflictException(this.tr('challenges.titleExists', { title }));
    }

    const normalizedTitleMatch = await this.challengeModel
      .findOne({ normalizedTitle })
      .lean()
      .exec();
    if (
      normalizedTitleMatch &&
      String((normalizedTitleMatch as any)._id) !== String(ignoreId || '')
    ) {
      throw new ConflictException(this.tr('challenges.titleExists', { title }));
    }
  ...
  async create(
    ...
    createdBy?: string,
    actorName?: string,
  ): Promise<{ challenge: ChallengeDocument; warnings: string[] }> {
  ...
  async update(
    id: string,
    dto: Partial<CreateChallengeDto>,
    actorId?: string,
    actorName?: string,
    ...
    for (const key of Object.keys(dto)) {
      if (
        JSON.stringify((existing as any)[key]) !==
  ...
  async incrementSolvedCount(id: string): Promise<void> {
    await this.challengeModel
      .updateOne({ _id: id }, { $inc: { solvedCount: 1 } })
      .exec();
  }
}
