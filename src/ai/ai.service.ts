  ...
  async generateChallenge(
    ...
    if (!this.groq) {
      throw new HttpException(
        'AI challenge generation is unavailable: GROQ_API_KEY is not configured.',
    ...
    for (let attempt = 1; attempt <= this.MAX_ATTEMPTS; attempt++) {
      try {
        this.logger.log(
          `[ChallengeGen] Attempt ${attempt}/${this.MAX_ATTEMPTS}`,
        );
        ...
        this.logger.debug(
          `[ChallengeGen] Raw response length: ${rawContent.length}`,
        );
        ...
        if (attempt < this.MAX_ATTEMPTS) {
          ...
          currentUserPrompt =
            baseUserPrompt +
            '\n\nWARNING: Your previous attempt (attempt ' +
            attempt +
            ') failed.\n' +
            'Reason: ' +
            errorMsg +
...
- If ANY mismatch exists, fix it before responding`;
  ...
  private extractJSON(raw: string): any {
    ...
    if (firstBrace === -1 || lastBrace === -1 || lastBrace <= firstBrace) {
      throw new Error(
        'NO_JSON_FOUND: Could not locate a JSON object in AI response',
      );
    }
  ...
  private validateSchema(data: any): void {
    ...
    for (const field of requiredStrings) {
      if (typeof data[field] !== 'string' || data[field].trim() === '') {
        throw new Error(
          'SCHEMA_INVALID: missing or empty field "' + field + '"',
        );
      }
    }
    ...
    if (!['easy', 'medium', 'hard'].includes(data.difficulty.toLowerCase())) {
      throw new Error(
        'SCHEMA_INVALID: difficulty must be easy/medium/hard, got "' +
          data.difficulty +
          '"',
      );
    }
  ...
  private validateEdgeCases(
    ...
    if (hasStringInputs) {
      ...
      hasEdgeCase = inputs.some(
        (i) =>
          typeof i === 'string' &&
          (i.length === 0 || i.length === 1 || new Set(i.split('')).size === 1),
      );
  ...
  private async verifyTestCases(
    ...
    try {
      ...
      for (let i = 0; i < executionResult.results.length; i++) {
        ...
        if (!r.passed) {
          if (r.error) {
            errors.push('testCase[' + i + ']: Runtime error — ' + r.error);
          } else {
            errors.push(
              'testCase[' +
                i +
                ...
                ', ' +
                'expected=' +
                JSON.stringify(testCases[i].expectedOutput) +
                ', ' +
                'actual=' +
                JSON.stringify(r.got ?? r.output),
            );
          }
        }
      }
